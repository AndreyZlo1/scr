...
local Config = {
	...
	FrameLookaheadCap= 0.045,  -- жёсткий потолок (сек), чтобы фриз не выстрелил абсурдно рано
	SmartDodgeDir = true,
	DodgeWallCheck = true,
	DodgeWallDist  = 8,
	...
	-- Универсальная контра для будущих стилей с M2GrantsIFrames (без правки движка). ВЫКЛ по
	-- умолчанию: включать осознанно, т.к. подходит не всякому стилю тактически.
	GenericIFrameCounter = false,
	...
	-- на дальней границе: i-frames поднимались за ~260мс до контакта и истекали через 40мс после.
	...
	-- получили причину «in-window но не выбран EDF» — фантомы вытесняли настоящий удар.
	...
	DecoySpeedTol   = 0.50,   -- допустимое отклонение track.Speed от 1.0 у ЧУЖОЙ атаки.
	                          -- Реплицированный трек реального врага всегда 1.00 (скорость
	                          -- удара сервер учитывает через рост в GetScaledHitboxDelay, а НЕ
	                          -- через Speed трека). Брейкер играл на 2.36 → отсекается.
...
local LEGACY_ATTACKS = {
	...
	-- ⇒ d = base+offset (WINDUP_EXTRA добавляет hitTimelineBase).
...
-- [V90] Аварийный маппинг имени анимации → id M2-варианта. Работает, только если живой
-- CombatConfig.GetStyleM2Variants недоступен (модуль не подгрузился). Живой конфиг приоритетнее.
...
local V93 = {
	...
	-- «скрипт плохо работает на маленьком фпс». frameDt — EMA дельты Heartbeat, lookahead —
...
local hitboxGeom = LPH_NO_VIRTUALIZE(function(th)
	...
	-- дальних, ломая ближних. В High forward входит только в reach (верхнюю границу), где
...
local willHitMe = LPH_NO_VIRTUALIZE(function(th)
	...
	-- 63/80/123 промахов, т.е. крупнейшая категория, и причину нельзя было увидеть.
...
-- тяжёлых с РАЗНОЙ HitboxDelay под одним kind="M2". Игра выбирает вариант третьим аргументом
...
local function resolveInfo(id, model)
	...
	--     (дамп M2_ModuleScript:1633 читает именно его через GetAttribute). Это пер-свинговая
...
local function hitTimelineBase(info, combo)
	...
	if info.t == "M2" then
		...
		if not cfgv then
			...
			-- 1) точная задержка варианта из LEGACY_ATTACKS (Ali Right=0.67 vs Left=0.53),
...
-- forward брался ТОЛЬКО из GetStyleHitboxForwardOffset (Ali M1 = 4.3), а лунж не учитывался вовсе.
...
-- Возвращает ключ стиля, если контра сейчас в принципе доступна, иначе nil.
local function counterStyle()
	...
	-- Универсальная ветка: любой будущий стиль, чья M2 даёт i-frames (без правки движка).
	if Config.GenericIFrameCounter then
		loadGameModules()
		if GameData.cfg and GameData.cfg.GetStyleBoolean then
			local ok, v = pcall(GameData.cfg.GetStyleBoolean, st, "M2GrantsIFrames", false)
			if ok and v == true then return st end
		end
	end
	return nil
end
...
-- время вокруг выстрела, чтобы серверная сторона увидела нужное направление при обработке.
...
local function fireBoxingCounter(th)
	...
	if cs and cs ~= "boxing" then
		...
		if hasVars then steerM2Variant(Config.AliM2Variant or "Right") end
	end
...
local function tryAliEvasiveCounter(now)
	...
	if not (tx and tx.confirmed) then return false end
	...
	-- окно: пока i-frames реально держатся (иначе это уже обычная M2 и упрётся в кулдаун)
	if now > (tx.hi or 0) then return false end
	local c = localChar(); if not c then return false end
	if c:GetAttribute("Equip") ~= true then return false end
	if c:GetAttribute("CombatAttacking") == true or c:GetAttribute("Stunned") == true then return false end
	...
	diagPush(("EVCOUNTER t=%.2f  %s  dist=%.1f  (Ali free M2, ignores M2 cooldown, range %.0f)")
		:format(now, best.name or "?", bestDist, range))
	return true
end
...
-- [V92] Поиск цели для мгновенной контры БЕЗ побочных эффектов. Вынесен из tryBoxingCounter,
...
local function counterCandidate(now)
	if not counterReady() then return nil end
	local myHRP = localHRP()
...
-- [V92] Мемоизация на кадр: гейты доджа спрашивают это несколько раз за кадр, а counterCandidate
...
local function counterPreemptsDodge(now)
	...
	State.counterPreemptFrame, State.counterPreemptVal = FrameId, best ~= nil
	...
	if best and now >= (State.lastPreemptLogAt or 0) + 0.5 then
		...
		diagPush(("COUNTER-PREEMPT t=%.2f  %s  dist=%.1f  (dodge skipped, our M2 grants i-frames)")
			:format(now, best.name or "?", bestDist or -1))
	end
...
State.ap = {
	...
	-- берутся ФИКСИРО��АННЫМ смещением от него + строгая проверка типов (см. getM1). Точный порядок
...
local refreshContact = LPH_NO_VIRTUALIZE(function(th)
	local now = os.clock()
	-- [V91] ЖИВОЙ ДОРЕЗОЛВ M2-ВАРИАНТА. Атрибут M2VariantId сервер выставляет и реплицирует,
	...
	-- т.е. на 140мс раньше при окне перфекта 125мс. Проверка стоит один раз на угрозу.
		...
		if okv and type(av) == "string" and av ~= "" then
			...
			if av ~= th.variant then
				...
				if type(newTL) == "number" and newTL > 0 then
					th.hitTL = newTL
					-- contact0 отсчитывается от detectClock: сдвигаем на ту же дельту, чтобы
...
local function onAttack(attackerHRP, info, model, id, track)
	...
	-- регистрировалась ПЕРВАЯ, вторая отбрасывалась через `return`. Но Anti-AutoParry работает
	...
	-- Теперь быстрый повтор НЕ отбрасывается: он регистрируется с меткой suspect=true, а решение
	...
	local th = {
		...
		-- [V91] combo и variant теперь ЖИВУТ НА УГРОЗЕ. combo нужен геометрии: студы лунжа
		-- (M1StepForwardStuds) заданы ПО НОМЕРУ УДАРА комбо — у Ali это {[1]=1.5, [3]=1.5}.
		-- variant нужен, чтобы реч тяжёлой считался по её настоящему варианту.
...
local function performDodge(now, reason, preferBack, force, bypassAutoOff)
	...
	-- задержка. С фиксированными 0.18 план расходился с фактом на ±150мс, из-за чего
...
local function updateDodgeTxn(now)
	...
	if not tx.confirmed and c and c:GetAttribute("IFRAMES") == true then
		...
		-- предсказанных КЛИЕНТСКИХ часах (сервер+oneWay), а IFRAMES мы наблюдаем через RTT
...
local schedulerStep = LPH_NO_VIRTUALIZE(function(now)
	...
	-- уже прилетевший удар считался покрываемым. Дэш не защищает назад во времени.
	...
	if clusterStrategy == "DODGE_M1_PARRY_M2" and dodgeReady() and canDodgeNow()
	   and not counterPreemptsDodge(now) then
	...
	if dodgeReady() and canDodgeNow() and #imminent >= 1 then
		...
		-- время» просто означает подождать следующий кадр. Даёт ~±150мс запаса с обеих сторон
	...
	-- стреляет внутри уже поднятых i-frames, т.е. не тратит ни кулдаун M2, ни защиту. Обычная
...
local function hookAnimator(animator)
	...
	animator.AnimationPlayed:Connect(function(track)
		...
		if Config.AntiDecoy and Config.DecoyHardDrop ~= false then
			...
			if okS and type(spd) == "number" then
				local tol = Config.DecoySpeedTol or 0.5
				if spd < (1 - tol) or spd > (1 + tol) then
					State.decoyDropped = (State.decoyDropped or 0) + 1
					if (nowd - (State.lastDecoyLog or 0)) > 1 then
						State.lastDecoyLog = nowd
						aclog(("[breaker] %s speed=%.2f (expect 1.00±%.2f) — phantom dropped x%d")
							:format(tostring(rec.model and rec.model.Name or "?"), spd, tol,
								State.decoyDropped))
					end
					return
				end
			...
			S[dk] = nowd
			-- ограничиваем рост таблицы (много игроков × много анимаций за сессию)
			State.decoySeenN = (State.decoySeenN or 0) + 1
			if State.decoySeenN > 256 then
				State.decoySeenN = 0
				for k, t in pairs(S) do if (nowd - t) > 8 then S[k] = nil end end
			end
		end
...
RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(hbDt)
	-- [V90] Дельта кадра (Heartbeat отдаёт её первым аргументом) → EMA → lookahead.
	...
	V93.lookahead = math.min(V93.frameDt * (Config.FrameLookahead or 0.5),
		Config.FrameLookaheadCap or 0.045)
...
return function(_Lib, _Core)
	...
	function M.buildUI(ctx)
		...
		-- навсегда, т.е. весь код был мёртвым. Здесь они выводятся в UI.
		apBox:Divider()
		...
		apBox:SubLabel({ Text = "ali style only\nsame idea as boxing counter: ali M2 grants i-frames, so ur own M2 replaces the parry" })
		...
		apBox:SubLabel({ Text = "bigger than boxing: ali M2 steps 2 studs forward on its own" })
		...
		apBox:SubLabel({ Text = "after a CONFIRMED dodge ali M2 ignores its 7s cooldown n reaches 22 studs\nfree hit from inside the i-frames" })
		apBox:Dropdown({
			Name = "Ali M2 Variant",
			Options = { "Right", "Left" },
			Default = Config.AliM2Variant or "Right",
			Callback = function(v)
				if type(v) == "string" and v ~= "" then Config.AliM2Variant = v end
			end,
		}, ctx.flag("AP_AliM2Variant"))
		apBox:SubLabel({ Text = "Right = x1.25 dmg + ragdoll (slower 0.67s)\nLeft = faster 0.53s, x0.8 dmg, no ragdoll\npicked by steering ur MoveDirection, server resolves it itself" })

		apBox:Divider()
		apBox:Header({ Name = "Any Style" })
		...
		apBox:SubLabel({ Text = "ur counter M2 already grants i-frames, so dont burn a dodge too\nunblockable grabs still always dodge" })
		boolToggle(apBox, "Generic i-Frame Counter", "Generic i-Frame Counter",
			function() return Config.GenericIFrameCounter end,
			function(v) Config.GenericIFrameCounter = v end)
		apBox:SubLabel({ Text = "any other style whose M2GrantsIFrames is true in the dump\noff by default, not every style wants this" })
