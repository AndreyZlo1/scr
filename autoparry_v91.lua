...
local Config = {
	...
	MatchWindow   = 1.30,
	-- [V125] ок������������о, в котором ВТОРОЙ (и далее) серверный OUT того же типа от того же врага
	-- считается доп-ударом ОДНОГО мультихит-сви��га (Boxing M2MultiHitCount=2 шлёт 2 события
	...
	--   2) FrameLookaheadCap=0.045 обрезал компенсацию: на 12 FPS (83��с кадр) нужно ~42мс, на
	...
	-- (TimePosition ползёт по чуть-чуть), ��тойл не де��екти��������������ся → contactAbs тикал к
	...
	-- Косвен��ое подтверждение из дампа: игра для своего же EvasiveCounter жёстко прописала
	-- VariantId = "Left" (CombatConfig.Styles.ali.EvasiveCounter) — ровно по ��той причине.
	...
	-- поэтому дедлайн здесь — тол��ко сетевое пле��о + этот ��апа��, а не наш hitbox delay.
	...
	-- AntiDecoyGap=0.12 этот поток НЕ ловил: 170мс > 120мс, т.е. брейкер шёл ровн�� над по��ог��м.
	-- ��ва независимых физических инварианта (оба проверены на реальных сессиях):
	...
	-- в середине burst, re-press �� BlockCooldown исключён.
	...
	-- (не скрываются), ESP визуально гладкий, а ��агрузк�� на вы��оком fps падает вд��ое+.
	VizMaxFPS     = 60,
...
-- [V151] UserInputService удалён: ни одной ссылки в файле (ввод идёт чер��з MacLib Keybind).
...
local State = {
	...
	threatNeutralized = 0,
	-- [V147] Анти-спам диаг�� гейта доджа: печатаем факт блокировки один раз, сбрасываем при
...
-- [V139/PERF] Два не��ав��симых источника ф��иза «на сбросе кэша» жили ЗДЕСЬ:
...
-- [V144/PERF] Точка входа всего диага (36 мест) — дешёвая функция, но вызовов мно��о.
...
-- ВАЖНО: модуль целико�� — одна гигантская функция, а в Luau лимит 200 жи��ых локалов на ��ункцию.
...
-- ═══════════════ [V161] ПОЧЕМУ «refused: CantAnything» — ЭТО НЕ БАГ СКРИПТА ═══════════════
-- Диаг 16:51:54 дважды показывает «BLOCK? refused: CantAnything» (t=225.55 и t=237.00) при живой
-- доказанной угрозе, и это выглядит как «скрипт ничего не делает». Дамп объясняет однозначно,
-- Block_ModuleScript:214 ApplyLocalGuardbreakLockout:
--     local v27 = CombatConfig.Shared.GuardbreakLockoutSeconds or 2
--     u1.SuppressBlocking(v27)
--     Character:SetAttribute("ParryBuffered", nil)
--     Character:SetAttribute("GuardBroken", true)
--     Character:SetAttribute("CantAnything", true)
--     Character:SetAttribute("Stunned", true)
-- После гардбрейка блок подавлен НА ДВЕ СЕКУНДЫ, и обойти это нечем: единственная ветка
-- Block.lua, разрешающая блок в стане (:368), требует `ParryBuffered == true`, а этот же
-- гардбрейк его только что снял (:224) — во всём дампе ParryBuffered нигде не ВЫСТАВЛЯЕТСЯ,
-- поэтому ветка мертва. Второй путь (:428-436) требует `Parried == true`, чего при гардбрейке нет.
-- Додж тоже недоступен: Evasive_ModuleScript отказывает по Stunned (:588), GuardBroken (:592) и
-- CantAnything (:600) — по всем трём флагам, которые гардбрейк ставит одновременно.
-- Вывод: эти 2 секунды беспомощности заложены игрой, и правильная цель — НЕ ДОПУСКАТЬ гардбрейка
-- (следить за стаминой блока), а не искать обход, которого не существует.
...
local function attackerYawRate(aHRP, flatLook)
	...
	if rec then
		...
		-- [V101] позиция/время ��рошлого кадра ��� для measured-closing (дельта дистанци��),
...
-- строко��ым атрибутом VictimSwingId. Если парт пересекается с нашим персонажем
...
local hitboxGeom = LPH_NO_VIRTUALIZE(function(th)
	...
	if meG then
		...
		if toMeG.Magnitude > 0.05 then
			...
			-- вре��я замаха (tHit до ~0.45с при скорости бега 16-28 ст��д/с) закрывает 8-12 студов —
			...
			-- прибавляет latVec — до WillHitLatCap=1.5 студа боком. Ито��овый |lead| снова
			...
			-- началом отсчёта для ox,oz на строке ~1778, и знак вектора «атакующий→��»
...
local willHitMe = LPH_NO_VIRTUALIZE(function(th)
	...
	-- ═══════ [V164] АВТОРИТЕТ ПЕРВЫМ: реальный оверлеп сервера НЕЛЬЗЯ вето-ить эвристикой ═══════
	-- Живой серверный хитбокс-парт, который пересекается с нами, — это ground truth: нас БЬЮТ.
	-- Раньше грубый фильтр «другого этажа» (MaxHeightDiff) стоял ВЫШЕ этой проверки, поэтому
	-- низкий MaxHeightDiff (в конфиге юзера 5.25, при дефолте 12 — артефакт старого пресета)
	-- мог молча выбросить ПОДТВЕРЖДЁННЫЙ оверлеп: willHitMe возвращал false с recognitionSource
	-- "y-diff", нажатия не было и ни строчки в лог. Проверяем истину до всякой эвристики.
	...
	-- Грубый отсев «другой этаж/уровень». Осмыслен ТОЛЬКО для геометрического прогноза ниже —
	-- реальный оверлеп уже вернул true выше. Server-proven свинг здесь не роняем никогда: сервер
	-- уже привязал этот свинг к нам, и высотная эвристика не вправе его отменять.
	-- [V165] Пол в 8 студов: сохранённый конфиг юзера несёт MaxHeightDiff=5.25 (артефакт старого
	-- пресета). При прыжках/лунжах вертикальная разница легко доходит до ~6-7 студов на ОДНОМ
	-- уровне, и слишком низкий порог молча выбросил бы реальный удар как «другой этаж». Настоящий
	-- разный этаж — это десятки студов, поэтому пол в 8 безопасен и лечит устаревшее значение,
	-- не трогая сам конфиг. Плюс серверно-подтверждённый свинг сюда всё равно не заходит (V164).
	local maxYd = math.max(Config.MaxHeightDiff or 12, 8)
	if not th.serverProven
	   and math.abs(myHRP.Position.Y - aHRP.Position.Y) > maxYd then
	...
	if mode == "High" then
		...
		th.geomFaceToMe = faceToMe
			...
			-- ═══════ [V165] РЕЧ ОБЯЗАН УЧИТЫВАТЬ ПОДХОД ВРАГА, А НЕ ТОЛЬКО ЛУНЖ ═══════
			-- КОРЕНЬ «скрипт игнорирует 1-2 удара, стоя на месте». В диаге 17:39 враг RoxyGin (Karate)
			-- бьёт M1 с dist2d=9.03 при reach=8.08 → OUT-OF-REACH → НЕ парируем → удар проходит →
			-- ragdoll/stun → каскад. Но Karate M1 НЕ лунжит (в дампе M1StepForwardStuds=nil,
			-- наследует default=nil, поэтому step=0.00 в логе — это ВЕРНО). Враг доводит удар не
			-- лунжем, а НОГАМИ: просто подходит. А `predA`/`lead` ограничены WillHitVelCap=2.0 студа
			-- и клампятся «не доходя до нас» (V109) — так что подход на 0.95 студа за 293мс
			-- (dt) в reach не попадал. Ровно то, что видит юзер: удар в упор проигнорирован.
			-- Расширяем ТОЛЬКО верхнюю границу reach на дистанцию, которую враг физически покроет
			-- к контакту со СВОЕЙ текущей скоростью В НАШУ сторону. Это не эвристика «доверия»:
			-- скорость измеряется, время до контакта известно, направление — на нас (иначе gate
			-- facing ниже отсечёт). Клампим, чтобы кратковременный всплеск скорости не растянул reach
			-- через всю арену. Расширение согласовано с принципом V160 «экстраполяция только расширяет».
			local approachAllow = 0
			if faceToMe >= faceFloor and dist2d > reach then
				...
				if dtc > 0 and dtc <= (Config.MaxWait or 1.2) then
					...
					if avOk and av then
						local velToMe = av.X * toMeX + av.Z * toMeZ   -- проекция скорости врага НА НАС
						if velToMe > 0.5 then
			...
			th.geomApproach = approachAllow   -- для диага GEOM-REJECT/TRACE-GEOM
			hit = (dist2d <= reach + approachAllow) and (faceToMe >= faceFloor)
	else
	...
	if not hit and Config.DeepDiag and not th.geomRejLogged then
		...
		local appr = th.geomApproach or 0   -- [V165] расширение reach за счёт подхода врага
			local reachEff = reachD + appr
			diagPush(("GEOM-REJECT t=%.3f %s %s(%s) c%s v%s mode=%s dt=%+.0fms | dist2d=%.2f reach=%.2f+appr%.2f=%.2f (fwd=%.2f step=%.2f halfD=%.2f pad=%.2f) %s | faceToMe=%+.2f floor=%+.2f %s | depth=%.2f side=%.2f/%.2f")
...
local function nextCombo(attacker)
	...
	-- КАЖДЫЙ свинг гонял полный pairs-обх��д ComboState. Тихая O(n)-д��градация на пустом месте,
...
-- Держать его = держать регистр главного чанка, а мы вылез��и за ��имит Luau 200 и получили
...
local function attackSpeedMult(model)
	...
	if mult == 1 then
		-- фолбэк: атрибу�� Height + формула из дампа CombatUtils
		local h
		pcall(function()
...
-- и парри срабатывал на ЧУЖОЙ блок. Собираем их явно и жёстко ис��лючаем из д��текта у��роз.
local BlockIds = {}
...
local resolveInfo = LPH_NO_VIRTUALIZE(function(id, model)
	...
	--  1) АТРИБУТ M2VariantId на персонаже а��акующего. Сервер выставляет его сам и реплицирует
...
--   → на HRP наве��ивае��ся LinearVelocity "CombatStepForwardLinearVelocity" на 0.14с,
...
local function playBlockAnim()
	...
	-- ЖИВЫМ поверх свинга и, б��дучи Action-приоритетом с бесконечным лупом, забирает вес за
	...
	-- Почему Test Swing был чистым: угроз нет → guard не поднят → тре��а "Blocking" не
...
-- [V91] КОНТРА ОБОБЩЕНА С BOXING НА ЛЮБОЙ СТИЛ�� С IFRAME-M2.
-- Почему это в��обще работает: смысл контры — вместо парирования пустить свою M2, у которой
...
--
-- ЧТО Д��ЛАЕМ: одна функция, которую ОБЯЗАНЫ вызвать все пути нашей M2 с i-frames.
-- ═══════════ [V147] ПОЧЕМУ БЫЛО «out of local registers» ═══════════
-- Luau даёт максимум 200 РЕГИСТРОВ (локальных переменных) на функци��, а тело файла — тоже
...
-- лимитом регистров, поэтому, как и V146/V151, держим функцию полем уже существ��ющей State.
...
-- ═══════════════════ [V161] PARRY-ONLY M2 ВЫВОДИТСЯ ИЗ ДАМПА, А НЕ ИЗ ИМЕНИ СТИЛЯ ═══════════════════
-- Было захардкожено ровно одно имя ("boxing"), поэтому Capoeira M2 уходила в dodge, хотя по
-- данным игры дожить до конца её удара додж НЕ МОЖЕТ. CombatConfig:451 capoeira:
--     M2HitboxDelay = 0.45, M2HitboxDuration = 0.32, M2Cooldown = 12,
--     M2RagdollLaunchStrength = 99, M2RagdollLaunchVerticalImpulse = 85.5, M2StrongKnockback = 22.5
-- при глобальном CombatConfig:99 Evasive.IFrameDuration = 0.3.
-- Хитбокс живёт 320мс, а неуязвимость доджа — 300мс: окно удара ДЛИННЕЕ окна i-frame, поэтому
-- любой идеально отцентрированный додж всё равно о��тавляет хвост удара непокрытым, и цена
-- промаха здесь максимальная в игре — ragdoll-подброс (99 + 85.5 вертикали). Парирование же
-- не зависит от длительности хитбокса вообще.
-- У Capoeira при этом НЕТ ни M2GrantsIFrames, ни M2MultiHitCount — то есть прежняя проверка на
-- Boxing её и не могла поймать: причина другая, а вывод тот же. Поэтому решаем по ТРЁМ признакам
-- из конфига, любой из которых делает dodge заведомо хуже парирования:
--   1) хитбокс не короче i-frame окна  → додж не покрывает удар целиком (capoeira);
--   2) M2MultiHitCount > 1            → несколько контактов на одну анимацию (boxing);
--   3) M2GrantsIFrames                → атакующий сам неуязвим, наш ответный M2 уходит в молоко.
-- Так любой будущий стиль с такими же свойствами попадает под политику без правки кода.
...
local function m2IsParryOnlyStyle(style)
	...
	local verdict = (st == "boxing")   -- fallback: сохраняем прежнее поведение, если конфиг не читается
	if cfg then
		-- (1) длительность хитбокса против длительности i-frame
		...
		-- (2)/(3) выделенных геттеров под эти два поля нет, но есть типизированные общие
		-- (CombatConfig:871 GetStyleBoolean, :GetStyleNumber) — они сами делают fallback на
		-- Styles.default, поэтому им и отдаём приоритет, а сырую таблицу держим как резерв.
		local multi
...
-- Прежнее имя сохранено: все существующие места вызова гейтят выбор dodge-vs-parry в Ali-путях,
-- и семантика (наш стиль Ali) там не меняется — расширился только список стилей ВРАГА.
...
function State.aliDodgeAbuseEligible(th, now, imminent, ifLat, ifDur)
	...
	-- ═══════ [V162] НЕ ЖЖЁМ ABUSE-ДОДЖ В ЗАВЕДОМЫЙ ОТКАЗ СЕРВЕРА ═══════
	-- ЭТО ПРИЧИНА «ALI COUNTER НЕ РАБОТАЕТ ВООБЩЕ». Цепочка контры такая:
	--     abuse-dodge → perfect dodge → серверный проц StyleEvasiveCounter → бесплатная M2.
	-- В диаге первое звено проваливалось, и дальше не шло НИЧЕГО (ни ALI-PERFECT-CONFIRM,
	-- ни ALI-EVCOUNTER-SEND за всю сессию):
	--     ALI-DODGE-ABUSE t=1738.89 target=SuperMegaCookie135/M1 s1 contactIn=266ms
	--     DODGE-REJECT/EARLY-FALLBACK t=1740.11 ali-dodge-abuse ack=1218ms IFRAMES not confirmed
	-- То есть Evasive был ОТКЛОНЁН сервером: атрибут IFRAMES так и не появился за 1218мс.
	-- Спасло нас парри (OUT ... PERFECT pressDt=132ms), а проц не наступил — доджа фактически не было.
	-- Дамп Evasive_ModuleScript отказывает по каждому из этих флагов ДО отправки:
	--     :588 Stunned | :592 GuardBroken | :600 CantAnything | :613 CombatAttacking
	-- причём :613 не делает поблажки ни на грант, ни на force. CombatAttacking выставляет наш же
	-- AutoPlay M1, поэтому «выстрел» abuse-доджа в этот момент гарантированно уходит в никуда и
	-- вместе с ним сгорает право на контру с кулдауном 6с. Проверяем состояние ДО решения.
	local meC = localChar()
	if meC then
...
local function fireBoxingCounter(th, targetDist)
	...
	if myHRP and aHRP and aHRP.Parent then
		...
		-- неверным. Длительность теперь выбирает��я по стилю и управляется отдельным UI slider.
	...
	-- получали ра��решение на додж — хотя i-frames от нашего же M2 в этот момент активны
...
local function tryAliEvasiveCounter(now)
	...
	-- Поэтому sender живёт до authoritative transaction deadline, не до предсказанного iframe края.
		...
		-- Проц — это ПРАВО на специал��ную M2, а не часть неуязвимости. В диаге он стабильно
...
local function counterCandidate(now, ignoreTransient)
	...
	-- [V91] реч контры зависит от стиля: Ali M2 сама довод��т на M2StepForwardStuds=2 студа
...
local function tryBoxingCounter(now)
	...
	-- ═══════════ [V162] КОНТРА ОТСТУПАЕТ ПО ДЕДЛАЙНУ ПАРРИ, А НЕ ПО ДЕДЛАЙНУ IFRAMES ═══════════
	-- ЭТО И ЕСТЬ «СКРИПТ ИГНОРИРУЕТ 2-3 УДАРА». Доказательство из диага — каждая угроза шла по
	-- одному и тому же сценарию: сначала окно парри открыто и НИЧЕГО не нажато
	--     PRESS-STARVED t=1715.659 M1 s1 dtWas=+181ms | step aborted BEFORE press branch
	--                   txnPending=false counterPend=false blockUp=true reason=-      ← всё чисто!
	-- и лишь через 111мс контра сдавалась, когда парировать уже поздно:
	--     COUNTER-SKIP/PARRY t=1715.77 contactIn=48ms need=131ms gate=IFRAMES-cannot-precede-contact
	-- Пары повторяются: 1716.342/1716.46, 1721.291/1721.42, 1724.657/1724.79, 1725.332/1725.46.
	-- Причи��а: пока контра держит угрозу кандидатом, угроза не попадает в EDF (строка ~5921
	-- «transaction owns this threat only»), а сдавалась контра по iframeLead ≈ 131мс — то есть
	-- ПОЗЖЕ момента, когда обязано уйти нажатие парри. В диаге целевой press был dt≈+177мс
	-- (TRACE-PRESS dt=+132ms lateBy=+45ms), поэтому к моменту отказа контры окно парри уже утекло.
	-- Дедлайн парри НАСТУПАЕТ РАНЬШЕ дедлайна контры, значит именно он и должен решать: если
	-- отправка контры не помещается до него — угроза сразу отдаётся парри, без 130мс простоя.
	...
	if contactIn <= yieldLead then
		-- Окончательно: ждать больше нечего, иначе парри тоже опоздает.
		best.counterCommittedToParry = true
		if not best.counterLateSkipLogged then
			best.counterLateSkipLogged = true
			diagPush(("COUNTER-SKIP/PARRY t=%.2f target=%s/%s contactIn=%.0fms need=%.0fms (iframe=%.0f parry=%.0f) gate=%s")
				:format(now, tostring(best.name), tostring(best.kind), contactIn * 1000,
...
-- [V140] Публи��уем на State, чтобы counterCandidate (объявлен ВЫШЕ) мог отфильтровать
...
-- Всё состояние И функции держим на State.ap — модуль впритык к лимиту 200 л��калов на функцию,
...
State.ap = {
	...
	tryM1Fn    = nil,    -- сам локальный tryM1() (upvalue #1 в OnM1Activated) — даёт bool успе��а
...
function State.ap.getM1()
	...
	if State.ap.m1 and type(debug) == "table" and type(debug.getupvalue) == "function" then
		pcall(function()
			...
			-- (function), поэ��ому это надёжный яко��ь. О���� него все индексы — фиксированным смещением,
...
function State.ap.fireM1Custom(char, model, wantCombo, ignoreRate, priority, dropGuard)
	...
	pcall(function()
		...
		-- перед Fire("M2","ServerCheck") на :1568, поэто��у M2 при этом флаге физически не уйдёт.
		...
		-- CombatAttacking он ��е распространяется, так что уступка обязательна, а не косметична.
	if Config.SkillAddon and Config.AliEvasiveCounter then
		...
		-- ═══════ [V162] НЕ НАЧИНАЕМ СВИНГ ПЕРЕД ABUSE-ДОДЖЕМ ═══════
		-- Парная правка к гейту "self-attacking" в aliDodgeAbuseEligible. Тот гейт запрещает жечь
		-- abuse-додж в заведомый отказ Evasive по CombatAttacking (дамп Evasive:613), но сам по себе
		-- он бы просто ОТКЛЮЧИЛ контру: AutoPlay свингует непрерывно, поэтому свободного окна не
		-- появилось бы никогда. Поэтому симме��рично: пока приближается server-proven угроза, под
		-- которую нужен abuse-додж, НОВЫЙ M1 не начинаем — уже начатый доигрывает сам.
		-- Окно намеренно узкое (до контакта не больше pre-окна доджа), чтобы не терять урон:
		-- M1 повторяется каждые ~90мс, а право на контру даётся раз в 6с (CombatConfig:261).
		if Config.AliDodgeAbuse and Config.AutoDodge then
			local cdA = State.aliM2CD
			-- Abuse-додж осмыслен только когда обычная M2 на кулдауне (иначе бьём ей напрямую).
		...
		-- ло��ное ощущение «снятых задержек» и второго владельца M1-машины.
		...
		-- Разница между двумя путям�� ровно одна: КОЛИЧЕСТВО вызовов.
...
-- реальный р��диус нашего M1 С УЧЁТОМ РОСТА (крупнее аватар → больше хитбокс/дос��аёт да����ше)
...
-- flat-дистанция до модели с ПИНГ-ПРЕДИКТОМ е�� позиции (сервер видит враг�� впереди нашего экрана)
...
function State.ap.fireM2(model, why, variant)
	...
	-- если interrupt-M2 её не отметит, counter выстрелит второй M2 в то же окно (о��е уйдут в
...
function State.ap.tryInterrupt(now, th, threatCount)
	...
	-- Но если второ�� враг далеко (> DodgeHorizon от now) — это будущая угроза, прерываем смело.
	...
	if useM2 then
		...
		-- разре��ала додж через 80мс: `INTERRUPT t=47430.40 via=M2+IF` → `DODGE t=47430.48`.
...
function State.ap.fireM1(model, why, priority, dropGuard)
		local ap = State.ap
		local now = os.clock()
		-- [V153] Гейт стоит и в общей обёртк��, чтобы fallback tryM1/OnM1Activated тоже не стал
...
function State.ap.step(now)
	...
	-- а fireM1 самогейтится на Blocking. Раньше step рон��л guard и делал `return` → первый добив
	...
	-- уже в этом кадре. Поэтому Н�� делаем return — сразу бьём. Deactivated и ServerCheck уходят на
...
-- То есть при ЖИВ��М гранте IFRAMECD физически не может быть true. Если он всё-таки true —
...
--   • БАЗ�� под грантом — пол, который игра сама себе пишет в u6 (:627) даже при активном
...
local function dodgeReady()
	local c = localChar()
	-- MinSpacing действует ВСЕГДА: это наш анти-спам, а не кулдаун игры. По�� грантом он и
...
-- force=true (blatant override): пропуска��м ТОЛЬКО софт-состояния (Stunned/CantAnything),
...
local onAttack = LPH_NO_VIRTUALIZE(function(attackerHRP, info, model, id, track)
	...
	-- пачкой с ��очти нулевым интервалом → это НЕ отдельные удары. Регистрируем только ПЕРВЫЙ
	...
	-- То ��е ломало комбо: угроза живёт ещё 350мс после к��нтакта, поэ��ому c3 «владел» атрибутом,
	...
	local th = {
		...
		-- [V91] combo и variant теперь Ж��ВУТ НА УГРОЗЕ. combo нужен г��ометрии: студы лунжа
...
-- гарантированно мог выдать StyleEvasiveCounter. Стена перед целью переводит �� side, затем away.
...
local function performDodge(now, reason, preferBack, force, bypassAutoOff, dodgeTarget)
	...
	-- гасит только додж (i-frames). Это обязат��льная защита, а не удобство, поэтому она обяза��а
	...
	-- [V146] ФИЗИЧЕСКОЕ ВЕТО. Стоит в мастер-ге��те, чтобы его не могла обойти НИ ОДНА ветка —
	...
	-- (см. updateDodgeTxn ниже), поэтому parry/EDF всё это время продолжае�� работать штатно.
...
local function updateDodgeTxn(now)
	...
	-- ═══════════ [V161] ЖДАТЬ ПРОЦ МОЖ��О ТОЛЬКО ЕСЛИ КОНТРА ВООБЩЕ ВКЛЮЧЕНА ═══════════
	-- РЕГРЕССИЯ V160: условие ниже не спрашивало Config.AliEvasiveCounter. При выключенной контре
	-- perfectConfirmed не выставляется НИКОГДА (его ставит только обработчик StyleEvasiveCounter,
	-- который при выключенной опции не подписан), поэтому awaitingProc оставался true до самого
	-- ackDeadline, транзакция жила ~600мс вместо untilAt≈492мс, и всё это время tx.pending
	-- держал угрозы «в обработке» — парирование не наступало. Ровно наблюдаемое «скрипт тупо
	-- ничего не делает, как будто чего-то ждёт», при том что контра была выключена.
...
local schedulerStep = LPH_NO_VIRTUALIZE(function(now)
	...
	-- удержание сильных ссыл��к на модели между кадрами. table.clear делает то же од��им вызовом.
	...
	V93.nearPressStamp = os.clock()
		...
		for i = #Threats, 1, -1 do
			...
			-- ═══════ [V150] КОРЕНЬ «удар уж�� сбит, а скрипт всё равно доджит» ═══════
			-- У записи угрозы не было НИ ОДНОГО поля ��ро то, что атакующему сорвали замах:
			-- ��сть resolved/staleTrack/feinted/dodged/coveredByDodge — и все они про НАС.
			...
			-- Снимаем её МОЛЧА (как resolved): это не промах, удар отработан ��арированием, и
			-- записывать его в MISS означало бы врат�� в статистику точности.
			...
			-- задоджили — это независимый пропус��. Логируем ТОЧН��Ю прич��ну, чтобы
			...
			elseif Config.DeepDiag and not th.pressed and not th.dodged and not th.deadLogged then
				...
				elseif th.enteredWindow then
					-- [V163] Эта строка ВРАЛА и стоила двух итераций отладки: она приписывала любой
					-- пропуск «конкуренции EDF», хотя реальных причин минимум три, и в диаге
					-- 17:39:27 настоящей была мигающая геометрия (press-ветка не выполнялась вообще),
					-- а не выбор другой цели. Теперь печатаем факты, а не догадку.
					reason = ("in-window но нажатия не было: threatens=%s geomLatched=%s blocked=%s")
						:format(tostring(th.threatens), tostring(th.geomLatched or false),
				...
				-- отличить «промах по тай��ингу» от «гейт сознательно держал нажатие» — именн�� эти
		...
		elseif not th.dodged then
			...
			-- ═══════ [V163] КОРЕНЬ «СКРИПТ ВООБЩЕ НЕ ПЫТАЕТСЯ ПАРИРОВАТЬ» ═══════
			-- Под `if threatens then` (ниже) лежит ВЕСЬ press-путь: расчёт pressAt, вход в окно и
			-- присвоение wantBlock. Значит один кадр с threatens=false молча выкидывает нажатие —
			-- ни BLOCK?, ни FACEWAIT, ни MISS в этот момент не печатаются. Именно это в диаге:
			--   TRACE-PRESS ... dt=+48ms lateBy=+141ms   (окно открылось на dt=+189ms!)
			--   dt на нажатии: 48/46/41/42/34/42мс при контактах 261..451мс, lateBy=126..141мс
			-- Опоздание ПОСТОЯННОЕ (~130мс) и не зависит от длины замаха — это не дрейф предсказания
			-- (predErr=+1мс, contactAbs стоял на месте), а фиксированное окно, в котором геометрия
			-- мигала и press-ветка не выполнялась. Обе успешные атаки (PERFECT) — те, где мигания
			-- не было: press ушёл на dt=+178 и +163мс, lateBy=-4 и -1мс.
			-- Почему мигает: willHitMe пересчитывается каждый кадр по живым позициям, а оба бойца
			-- прыгают и вращаются (в диаге modelY гуляет 5.04..5.81). Защёлка V154 (GEOM-STICKY)
			-- ровно от этого и делалась, но она внутри геометрии ПОСЛЕ ранних `return false`
			-- (например вертикальный зазор `math.abs(myAt.Y - origin.Y) > halfH`), поэтому такие
			-- кадры её обходят — в логе видно, как GEOM-STICKY спасает OUT-OF-REACH, но ничего не
			-- знает про эти отказы.
			-- Правильный уровень — здесь: если сервер уже ПОДТВЕРДИЛ свинг и геометрия хотя бы раз
			-- признала его угрозой, то до контакта отменять его нельзя. Сервер к удару уже
			-- привязался, наша задача — успеть нажать. Это закрывает все ранние return разом.
			-- Финты/stale/neutralized сюда не доходят: их удаляют ветки выше (5960-5976).
			   ...
			   and (th.contactAbs - now) > -(Config.HoldAfter or 0.12) then
				...
				if Config.DeepDiag and (not th.geomLatchLogAt or now - th.geomLatchLogAt > 0.10) then
					...
					diagPush(("GEOM-LATCH t=%.3f %s %s s%d proven-swing un-threatened by live geometry → HELD"
						.. " | contactIn=%+.0fms src=%s")
						:format(now, tostring(th.name), tostring(th.kind), th.strike or 1,
		...
		if Config.SkillAddon and Config.SA_HakariRead and th.kind == "M2"
			...
			hold = hold + w
					...
					-- врагами. Компенсация задержки — толь��о физическая (lead + uplink + velLead).
				...
				-- Читает vizGate: п��ка защита в работе, ESP не претендует н�� бюджет кадра.
				...
				if now >= pressAtQ and now <= holdEnd then
					...
					-- снача��а берём угроз�� без н��жати�� (unpressed), сред��� ��их — с самым
	...
	-- (Config.MustDodge) остаётся — и�� и отключается эта защита при желании.
	...
	if dodgeReady() and canDodgeNow() and #imminent >= 1 then
		...
		-- [V154/ALI-DODGE-ABUSE] Это единственный намеренный dodge-вместо-parry пр�� доступном
		-- блоке. Он разрешён лишь на известном остатке M2 cooldown >1с и при полном покрытии всех
		-- угроз внутр��нней частью iframe. Для Boxing multi-hit первый strike сюда не проходит.
		...
		if Config.DodgeCenter then
			...
			-- (источник A = GetNetworkPing()*2) ⇒ up ≈ RTT. Сервер поднимает IFRAMES чере��
			-- oneWay по��ле отправки, iframe жив��т IFrameDur; серверный контакт наступает на
			...
			if soonestDt <= (fireLead + up + (V93.lookahead or 0)) then
				...
				-- Пока parry доступен (canBlockNow) — блокируем/перфектим ВСЁ, что блокир��емо, и НЕ
	...
	-- ═══════ [V161] ВНИМАНИЕ: ЭТА ЗАЩИТА ПОЧТИ НАВЕРНЯКА НЕ СРАБАТЫВАЕТ ═══════
	-- Проверено по дампу: во ВСЁМ клиентском коде нет ни одного SetAttribute со стаминой блока
	-- (grep по Stamina/Posture/BlockHealth даёт только SetAttribute("GuardBroken")), а
	-- CombatConfig.Shared.GuardbreakStaminaThreshold=10 не читается ни одной клиентской строкой.
	-- Значит стамина guard живёт ТОЛЬКО на сервере и клиенту не репликуется, blockStamina()
	-- возвращает nil, и ветка ниже никогда не даёт guardbreak-save. Подтверждение в диаге: гардбрейк
	-- случился (два «refused: CantAnything» = ApplyLocalGuardbreakLockout), но ни одного
	-- guardbreak-save перед ним не было.
	-- Единственный клиентский сигнал — уже СВЕРШИВШИЙСЯ факт: M1.lua:634 / M2.lua:1682 вызывают
	-- Block.ApplyLocalGuardbreakLockout() по СЕТЕВОМУ событию GuardBroken от сервера, то есть
	-- предск��зать пролом заранее клиент не может в принципе.
	-- Ветку оставляю: она безвредна и сработает на стилях/режимах, где атрибут всё же появится.
	-- Честный вывод: полноценная защита от гардбрейка требует учёта числа подряд заблокированных
	-- ударов (эвристика), а не отсутствующего атрибута — это отдельная задача, не молчаливый «фикс».
	if not overloaded and Config.GuardbreakProtect then
		...
		if st and st <= Config.StaminaFloor then
			overloaded, why = true, ("guardbreak-save(st=%.0f)"):format(st)
				...
				-- [V92] контра приоритетнее: она и защища��т (M2GrantsIFrames), и бьёт.
	...
	if turnTo and turnTo.attackerHRP then
		...
		-- а пакет летит к серверу ~пол-RTT. Значит жёстко доворачиваемся заранее — за (о��но + RTT)
	...
	if wantBlock then
		...
		-- этой игре НА��РАВЛЕННЫЙ: сервер отклоняет парри, если жертва ��мот��ит спиной к атакующему
		-- (в логах face=-0.99 BACK! на проваленных парри). Но ло��ально на����атие ��сё равно ж��ёт
		...
		if not wantBlock.pressed then
			...
			elseif State.blockedReason then
				...
				-- ═══════ [V161] ГАРДБРЕЙК = ОКНО, В КОТОРОМ НЕДОСТУПНО ВООБЩЕ ВСЁ ═══════
				-- Здесь была попытка увести угрозу в додж, раз блок подавлен. ДАМП ЕЁ ОПРОВЕРГАЕТ:
				-- предполётные гейты Evasive_ModuleScript отказывают по каждому из флагов, которые
				-- гардбрейк выставляет одновременно (Block.lua:214 ставит GuardBroken+CantAnything+Stunned):
				--     :588  if u50:GetAttribute("Stunned") then ... return
				--     :592  if u50:GetAttribute("GuardBroken") == true then ... return
				--     :600  if u50:GetAttribute("CantAnything") and not CombatRecovery then ... return
				-- То есть GuardbreakLockoutSeconds=2с — окно намеренной беспомощности: ни блок, ни
				-- парри, ни додж не уйдут. Логировать причину один раз (выше) — единственное
				-- корректное поведение; любые нажатия здесь заведомо отклоняются сервером.
				-- Вывод для приоритетов: гардбрейк нужно НЕ ДОПУСКАТЬ (не переблокировать в стамину),
				-- а не «обходить» — обхода не существует.
			end
		end
...
local function onOutcome(attacker, result, kind, eventClock)
	...
	-- [V163] Метка hint ниже считается от trueGap, но печаталась сразу ПОСЛЕ pressDt, из-за чего
	-- один и тот же pressDt=178мс выглядел то «EARLY(>150)», то «LATE(<50)» (диаг 17:39:27,
	-- строки OUT t=3096.14 и t=3096.52). Это сбивало диагностику: метка описывает зазор guard-up,
	-- а не момент нажатия. Приклеиваем её к trueGap, чтобы строка перестала противоречить себе.
	...
	diagPush(("OUT    t=%.2f  %s  %s(c%d,s%d)  %-10s  meas=%.0fms pred=%.0fms predErr=%+.0fms resAvg=%+.0fms(n=%d) | blockGap=%s guard=%s pressDt=%s%s | face=%s%s spd=%.2f ping=%.0f")
...
local function hookAnimator(animator)
	...
	animator.AnimationPlayed:Connect(function(track)
		...
		-- [V85] защитная анимация вра��а (блок/парри/perfect) — это НЕ входящая атака, ��е парир��ем.
		...
		if Config.AntiDecoy and Config.DecoyHardDrop ~= false then
			...
			-- счётчик всё ��авно сбрасывался в 0 → та же дорогая развёртка запускалась снова через
...
-- из-за ч��го мог держать guard и мешать. Парри теперь
...
local function captureIdleId(animator)
	...
	-- доверяем ��ахвату только когда стоим (иначе доминирующий looped-трек = walk)
...
local function toggleDesyncTest()
	...
	if DesyncTest.on then
		...
		-- ма��сима��ьный приорите��, чтобы перебивать walk/run (Movement) — берём Action4 если есть
...
-- decoy-overlay реально загрязняет чужой детект (�� не только крутится локально).
...
-- [V75] КРОСС-КЛИЕНТНАЯ ПРОВЕРКА (отвечает на "ка�� это видят ��ругие игроки").
...
function AnimLib.desyncOwnTrack(track, id, animator)
	...
	-- дё��гался. Пропускаем ��аши собственные decoy-треки — трогаем только реальные атаки.
		...
		if ok and kind then
			...
			local now = os.clock()
				if kind == "attack" then
					...
					State.attackBusyUntil = now + Config.SelfBusyDur   -- [V117] busy из-за ��ТАКИ
...
;(RunService.PreSimulation or RunService.Heartbeat):Connect(LPH_NO_VIRTUALIZE(function(hbDt)
	...
	-- сервер резолвит вариант из РЕПЛИЦИР��ВАННОГО MoveDirection при обработке нашего ServerCheck —
...
-- [V112] PERF: ПЕРСИ��ТЕНТНЫЕ scratch-буферы для точек кольца/конуса. Раньше drawFlatRing де��ал
-- `wpts={}` и drawTargetHitbox — `wArc={}`,`a2d={}`,`az={}` НА КАЖДЫЙ кадр отр��совки = 4 таблицы +
-- ~100 Vector2/Vector3 а��локаций/кадр → GC-дёрганье (главная оставшаяся прич��на «лагает»). Теперь
...
vizUpdate = LPH_NO_VIRTUALIZE(function(dt)
	...
	-- перерисовка (~280 операций проекции + пулы Drawing) платитс�� КАЖДЫЙ кадр — ровно тогда,
	-- когда бюджета и так нет. ESP усугублял просадку, из-за которой сам ��е и вызывался.
	-- Привязываем ин��ервал к РЕАЛЬНОЙ дельте кадра: на 20 FPS рисуем раз в ~2 кадра вместо
	...
	-- отдаётся парирован��ю: ESP не рисуется вообще. Это те 1–2 кадра, где точнос��ь тайминга
...
local applyFacing = LPH_NO_VIRTUALIZE(function()
	...
	if lead > 0 then
		...
		-- В УПОР (радиальный) больше НЕ съедает бюджет боковой коррекц��и (толчок влево/впр��во).
...
return function(_Lib, _Core)
	...
	function M.buildUI(ctx)
		...
		apBox:Dropdown({
			...
			-- пока движок работал по "Left" — рассинхрон UI и фактиче��кого поведения.
