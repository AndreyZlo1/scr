...
return function(Lib, Core)
    ...
    -- [V149] swap убран (подмена p55 удалена как инертная), вместо него:
    --   restart — сколько раз реально выдали Stop+Start после 3-го удара;
    --   rsSkip  — сколько раз рестарт ОТКЛОНЁН гейтами (min-interval или hold уже отпущен).
    -- Разделение важно: restart=0 при rsSkip>0 означает «механика упирается в гейт», а не
    -- «механика не работает» — это разные причины и лечатся по-разному.
    local _ndStat = { gate = 0, parry = 0, block = 0, calls = 0, restart = 0, rsSkip = 0 }
    -- Буфер последовательности номеров ударов от сервера (см. хук OnHoldSwing).
    local _ndSeq = {}
    local _ndSaid = {}
    ...
    -- [V112] ����золвер M1 → tryM1 → индексы гейтов. Выполняется ОДИН раз (ленивая карта).
    local _tryM1 = nil
    ...
    --    конца сессии u21/u32/u33 не т��огались НИКОГДА, а cidx превращался в `true`, после
    ...
    -- ══════��════════════════════════════════════════════════════════════════════════
    -- [V149] Рестарт серии M1: Hold("Stop") + Hold("Start") после 3-го серверного удара.
    -- ═══════════════════════════════════════════════════════════════════════════════
    -- Лимиты берём из ИГРОВОЙ таблицы Shared.Network.CombatRemoteLimits, а не выдумываем.
    -- Для нашей пары ключей там (строки 12-16, 26-28, 37-39, 67-70, 90-92):
    --   ClientMinInterval:            M1.HoldActivated 0.1с,  M1.HoldDeactivated 0.05с
    --   ClientSustainedMaxPerSecond:  оба по 5/с
    --   ServerMinInterval:            0.1с / 0.05с
    --   ServerSustainedMax (окно 1с): оба low=8, mid=10
    --   MonitoredKeys:                оба true → анти-чит эти ключи ОТСЛЕЖИВАЕТ
    -- Наш темп: свинги идут раз в ~0.45с (AttackDuration), цикл из 3 ударов ≈ 1.35с, значит
    -- ~0.74 Stop+Start в секунду — с большим запасом под 5/с и 8/с. Но гейт по минимальному
    -- интервалу всё равно ставим: без него любая аномалия темпа свингов (лаг, несколько
    ...
    local function _ndRestartMinGap()
        ...
        local lim = _ndRestartLimits or nil
        local a, d = 0.1, 0.05
        if lim and type(lim.ClientMinInterval) == "table" then
            a = tonumber(lim.ClientMinInterval["M1.HoldActivated"]) or a
            d = tonumber(lim.ClientMinInterval["M1.HoldDeactivated"]) or d
        end
        return math.max(a, d)
    end
    local function _ndRequestComboRestart()
        if _ndRestartBusy then return end
        local m1 = _ndM1Mod
        if not m1 or type(m1.Hold) ~= "function" then return end
        local now = os.clock()
        local gap = _ndRestartMinGap()
        if now - _ndLastRestart < gap then
            _ndStat.rsSkip = (_ndStat.rsSkip or 0) + 1
            return
        end
        local ch = LocalPlayer.Character
        -- Гейт «пользователь всё ещё держит удар». M1Hold — атрибут самой игры: Hold("Start")
        -- ставит его в true (:540), stopHoldIfActive снимает в nil (:172). Без этой проверки
        -- мы могли бы выдать Start уже ПОСЛЕ того, как игрок отпустил кнопку, и персонаж
        -- продолжил бы махать сам — это ровно то поведение, которое выглядит как читерский
        -- автоатак и ловится анти-читом.
        if not ch or ch:GetAttribute("M1Hold") ~= true then
            _ndStat.rsSkip = (_ndStat.rsSkip or 0) + 1
            return
        end
        _ndRestartBusy = true
        _ndLastRestart = now
        -- Отдельный поток: внутри хука OnHoldSwing нельзя ни ждать, ни дёргать Hold — сначала
        -- оригинал должен доработать текущий 3-й свинг (шаг вперёд, таймеры).
        task.defer(function()
            local okAll = pcall(function()
                m1.Hold("Stop")
                -- Пауза между Stop и Start: даём серверу зарегистрировать закрытие серии
                -- раньше её открытия. Ключи разные, поэтому в общий min-interval не упираемся.
                task.wait(_ndRestartMinGap())
                -- Start выдаём ТОЛЬКО если персонаж жив и hold не поднят кем-то ещё за эту
                -- паузу. Условие было бы инвертированным, если проверять лишь `== true`:
                -- при ch2 == nil (персонаж умер/перезагрузился) мы бы всё равно дёрнули Start.
                local ch2 = LocalPlayer.Character
                if not ch2 or ch2:GetAttribute("M1Hold") == true then return end
                -- ОГОВОРКА, которую важно знать: после нашего Stop атрибут M1Hold снят нами,
                -- поэтому «держит ли игрок кнопку ПРЯМО СЕЙЧАС» здесь уже не прочитать. Окно
                -- неопределённости равно паузе (0.1с). Если отпустить удар точно в этот
                -- момент, персонаж сделает один лишний замах — это цена механики, а не баг.
                m1.Hold("Start")
            end)
            _ndRestartBusy = false
            if okAll then
                _ndStat.restart = (_ndStat.restart or 0) + 1
                if _ndStat.restart <= 3 then
                    _ndSay("rs" .. tostring(_ndStat.restart),
                        "рестарт серии #%d: Stop+Start после 3-го удара, пауза %.2fс. "
                            .. "Ждём в логе p55=1 вместо 4",
                        _ndStat.restart, _ndRestartMinGap())
                end
            else
                _ndSay("rsfail", "рестарт серии FAIL (Hold Stop/Start бросил ошибку)")
            end
        end)
    end
    ...
    local function _ndInstallComboHook()
        ...
        local ok, err = pcall(function()
            _ndOrigSwing = hookfunction(m1.OnHoldSwing, function(p55, p56, ...)
                ...
                -- [V149] Копим ПОСЛЕДОВАТЕЛЬНОСТЬ номеров и печатаем её одной строкой на
                -- каждые 6 свингов. Именно последовательность отвечает на вопрос, работает ли
                -- рестарт: без механики ждём 1,2,3,4,1,2 — с механикой 1,2,3,1,2,3.
                -- Построчная печать для этого не годится (в логе V148 её сносило потоком stat).
                _ndSeq[#_ndSeq + 1] = tostring(p55)
                if #_ndSeq >= 6 then
                    _ndSay(nil, "последовательность p55: %s", table.concat(_ndSeq, ","))
                    _ndSeq = {}
                end
                -- ═══════════════════════════════════════════════════════════════════════
                -- [V149] ПОДМЕНА p55 3→4 УДАЛЕНА — ВАШ ЛОГ ДОКАЗАЛ, ЧТО ОНА ИНЕРТНА
                -- ═══════════════════════════════════════════════════════════════════════
                -- В логе V148: `p55 3->4 (#1..#3): шаг вперёд 0 -> 0 с��удов` и swap34 дорос
                -- до 5 — при нулевой разнице в поведении. Разбор по каждому потребителю v57
                -- внутри OnHoldSwing (M1.lua:499-515) объясняет, почему иначе и быть не могло:
                --   u19 = v57                          → живых читателей НЕТ (все в tryM1)
                --   ApplyM1StepForward(style, ch, v57) → 0 студов и для 3, и для 4: у текущего
                --                                        стиля нет ключей в M1StepForwardStuds
                --   scheduleM1SwingTimers(v57, p56)    → пишет u21 (мёртв) и таймер resetCombo
                -- То есть менять номер удара на клиенте бессмысленно: он ничего не решает.
                -- Заодно снят и риск, о котором я предупреждал (окно 0.45→1.25с).
                --
                -- ═══════════════════════════════════════════════════════════════════════
                -- [V149] РЕСЕТ КОМБО НА 3-М УДАРЕ — ВАШ ЗАПРОС, ЕДИНСТВЕННЫЙ ЖИВОЙ ПУТЬ
                -- ═══════════════════════════════════════════════════════════════════════
                -- Сначала честно про клиентский resetCombo (M1.lua:247-253), который и печатает
                -- `combo reset` в вашей консоли. Он делает ровно три записи:
                --     u19 = 0;  u21 = true;  u20 = nil
                -- Живых читателей нет ни у одной: u19 читается только в tryM1 (:460-486),
                -- u21 — только в `if not u21` на :350, тоже внутри tryM1, u20 — лишь хэндл
                -- задачи для отмены. Поэтому вызвать resetCombo раньше (на 3-м ударе) — значит
                -- напечатать лишнюю строку в лог и не изменить в игре НИЧЕГО. Второй источник
                -- `combo reset` (:563, внутри ServerResponse) вообще недостижим: он под
                -- `v65 = u27[p62]`, а u27 заполняется только на :486 — снова в мёртвой tryM1.
                --
                -- Номер удара приходит remote'ом от СЕРВЕРА, это подтверждено:
                --   CombatClientRemote.On("CombatM1HoldSwing", …) → dispatchCombat("M1",
                --   "OnHoldSwing", p114, p115)   (CombatReplicatorClient:717-720)
                -- Значит сбросить комбо можно только заставив сервер начать серию заново.
                -- Клиент влияет на сервер по M1 ровно двумя сообщениями (M1.lua:517-544, :162-186):
                --   Hold("Start") → Fire("M1","HoldActivated")    (гейт u23: повторный Start
                --                                                  без Stop игнорируется)
                --   Hold("Stop")  → Fire("M1","HoldDeactivated")
                -- Отсюда механика: поймав 3-й серверный свинг, делаем Stop+Start — сервер
                -- закрывает текущую серию и следующим свингом присылает p55=1, то есть медленный
                -- 4-й финишер (FinisherCooldown 1.25с против AttackDuration 0.45с, :305) не
                -- наступает. Это не обход задержки, а смена серии; проверяется по логу
                -- последовательности p55 (ждём 1,2,3,1,2,3 вместо 1,2,3,4).
                -- ═══════════════════════════════════════════════════════════════════════
                -- [V150] РЕСТАРТ СЕРИИ (Stop+Start) ОТКЛЮЧЁН — И ВОТ ПОЧЕМУ
                -- ═══════════════════════════════════════════════════════════════════════
                -- Я добавил его в V149 без доказательства эффекта, хотя он шлёт remote на
                -- ключах из MonitoredKeys. Это была та самая догадка, выданная за решение.
                -- Отключаю, потому что рабочий путь ускорения M1 в проекте УЖЕ ЕСТЬ и он
                -- другой: autoparry.lua:fireM1Custom (V105) не просит игру ударить, а сам
                -- строит удар — двигает гейты через debug.setupvalue, играет родную анимацию
                -- и шлёт ServerCheck напрямую в ServerRemote, минуя клиентский троттл. Именно
                -- это и есть аналог «сказать спринту, что стамина полная»: логика удара
                -- воспроизводится клиентом, а не выпрашивается у сервера.
                -- Дублировать ту же задачу из movement.lua вредно: два независимых источника
                -- M1-сообщений гарантированно рассинхронят номер комбо и упрутся в лимиты.
                -- Оставляю только телеметрию последовательности p55 — она бесплатная и
                -- показывает, что реально присылает сервер.
                _ = _ndRequestComboRestart   -- ссылка сохранена: механика ниже не вызывается
                return _ndOrigSwing(p55, p56, ...)
            end)
        end)
    ...
    --     ни на что. Оставшиеся — это СОСТОЯНИЯ (ле��ишь/схвачен), а не задержки, поэтому
    ...
    local function _animFolder()
        ...
        local folder, style
        pcall(function()
            folder = cau.GetCombatAnimsFolderForPlayer(LocalPlayer)
            if cau.GetPlayerCombatStyle then
                style = string.lower(tostring(cau.GetPlayerCombatStyle(LocalPlayer)))
            end
        end)
        return folder, style
    end
    ...
    local function driveNoDelay()
        ...
        -- ударов и ��спел ли сервер переписать значение — на следующем же кадре u19 снова 0,
    ...
    -- Хуков здесь боль��е НЕТ: ни task.delay, ни Animator-коннекта. Респавн ничего не
    ...
    -- детерминирова��а и коллизия «Cooldown 1.5 против множителя 1.5» невозможна в принципе.
    local EV_UPV = {
    ...
    local function mapEvasive()
        ...
        if hasDebugUpvalues() then
            ...
            -- код мог случайно записать в спис��к сам Cooldown или множитель.
    ...
    local function restoreDodge()
        ...
        -- заблуждение, из которого вырос баг со ско��остью дэша.
        local cfg = getCombatConfig()
    ...
    -- «накапливающиеся лаги» и падения: чем дольше се��сия и чем больше объектов, тем
    ...
    -- п��иском по всему дампу). Внутренние вызовы самого модуля тоже идут через `u1.SetBlur`
    ...
    -- ═══════ [V127] RESPAWN / AUTO RESPAWN УДАЛЁН ПОЛНОСТЬЮ (861 строка) ═══════
    -- Удалено по твоему указанию, и оно обосновано. Ниже — факты, чтобы никто (включая меня)
    -- не нача�� городить это заново. История версий V112-V126 намеренно стёрта вместе с кодом:
    -- она вся описывала пути, которые доказанно не работают.
    --
    -- ЧТО ПОКАЗАЛ ФИНАЛЬНЫЙ ДИАГ:
    --     state: Freefall | Downed: true | Dead: false
    --     Kill[Kill replicated] DeadAttr=false | LoadChar[invoked ok] -> no CharacterAdded
    -- То есть `replicatesignal(Player.Kill)` РЕАЛЬНО дошёл до сервера, но перевёл тебя лишь в
    -- `Downed=true` (HP в 0, падение) — а НЕ в `Dead=true`. Ты говорил это с самого начала:
    -- «это не смерть по мнению игры». Ты был прав, а я трижды принимал «вызов прошёл» за
    -- «сервер согласился».
    --
    -- ПОЧЕМУ ДРУГОГО СИГНАЛА НЕ СУЩЕСТВУЕТ (главный аргумент, проверяемый в дампе):
    --     grep 'SetAttribute("Dead"' по всему дампу → 0 совпадений
    --     grep 'GetAttribute("Dead"' по всему дампу → 19 совпадений
    -- Атрибут `Dead` только ЧИТАЕТСЯ клиентом и нигде им не ставится, то есть его выставляет
    -- исключительно сервер. А респавн выдаётся только при `Dead == true`
    -- (dumped/PlayerScripts/respawnstuff_LocalScript.lua:398 → :216 dorespawn → :228 RespawnRE).
    -- Значит дело не в том, что мы «не нашли нужный сигнал»: `Dead` вообще не управляется
    -- сигналами. Переход Downed → Dead — это серверное решение (таймер добития/реанимация),
    -- и никакой клиентский вызов его не подменит. Атрибуты, поставленные сервером, клиент
    -- изменить с репликацией не может — это ограничение самого Roblox, не игры.
    --
    -- Cmdr тоже закрыт: команда `respawnServer` делает настоящий `v:LoadCharacter()`, но
    -- CmdrAdminShared/CmdrShared:11 гейтит её жёстким списком UserId админов.
    --
    -- ИТОГ: возрождение здесь невозможно форсировать с клиента в принципе. Подъём из ragdoll —
    -- отдельная и РАБОЧАЯ задача, её делает Anti-Ragdoll ниже (ChangeState(GettingUp)).
        ...
        feature(sCbt, {
            ...
            set = function(v)
                ...
                if v then
                    ...
                    -- тумблере не вызывается вообще (проверка на :2090), поэтому ветка
                    -- восстановления внутри него до выключения не доживает.
                    _animRestore()
                    notify("No Delay", "Disabled")
                end
            end,
            -- [V151] Описание переписано под реальность. Прежний текст обещал «clears M1
