...
local Config = {
	...
	MultiThreatMinN   = 2,      -- со скольких одноврем��нных угроз ��ключать held-��еж��м
...
local function loadGameModules()
	...
	pcall(function()
		...
		-- Держим оба живыми: ��ардкод молча разойдётся с игрой при первом же балансном патче.
...
-- «вместо парирования МОМЕНТАЛЬНО бит�� M2, если стиль Boxing, враг атаковал в радиусе 5.5 и
...
-- функция (главный чанк). Top-level локалей здесь уже было ровно у предела:
...
--      i-frames до контакта — проигранная гонка всё равно не наказыв��ется.
...
local function dodgeReady()
	...
	if evasiveGranted() then
		...
		-- уже обнуляется при первом же подтверждении (:4676), поэтому счётчик не залипает:
		...
		if since < grantFloor then
			if not State.dodgeGateSaid then
				State.dodgeGateSaid = true
				diagPush(("DODGE-GATE  грант активен → пол %.2fс (DashDuration+ConfirmTimeout), прошло %.2fс")
...
local schedulerStep = LPH_NO_VIRTUALIZE(function(now)
	...
	if turnTo and turnTo.attackerHRP then
		...
		-- до контак��а. В мультибое (2+) — всегда hard, чтобы мг��овенно пе��екидываться ме��ду целями
